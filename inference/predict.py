    ...
    try:
        ...
        while end_ctr != num_processes:
            ...
            if item == "end":
                ...
                print('_______________end of process____________________')
                yield item
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz,
                  num_threads_preprocessing, num_threads_nifti_save,
                  segs_from_prev_stage=None, do_tta=True,
    ...
    # if not overwrite, filter files have been predicted
...
def predict_from_folder(model, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
    ...
    # So if your data are with wrong name, your predations's name will bias.
    case_ids = get_caseIDs_from_splitted_dataset_folder(input_folder)

    ### output files' path $$$ path + case name + '.nii.gz' ###
    output_files = [join(output_folder, i + ".nii.gz") for i in case_ids]

    ### all files of all case names ###
    all_files = subfiles(input_folder, suffix=".nii.gz", join=False, sort=True)
    ### list of lists of path+filename ###
    ...
    if lowres_segmentations is not None:
        ...
        lowres_segmentations = lowres_segmentations[part_id::num_parts] ### begin with part_id, get out one from every num_parts items
    else:
        lowres_segmentations = None

    print('------------------------------------------- 11')
    a= predict_cases(model,
                         ...
                         lowres_segmentations,               # low res pred path + case name + '.nii.gz'
                         tta,
